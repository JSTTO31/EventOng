<script setup>
import useEventForm from '@/Composables/useEventEditForm'
...
const props = defineProps(['locations', 'organizers', 'event'])
const {status, flat, mode, form, submit} = useEventForm(props.event)
const $page = usePage()
...
document.addEventListener('inertia:before', () => {
    ...
    return confirm('You have unsaved work, do you want to discard it?')
}, {once: true})
</script>
...
<template>
    <AuthenticatedLayout :show-app-bar="false" title="Edit event">
        <ShowLoading :showDialog="form.processing"></ShowLoading>
        <v-app-bar class="px-5" :flat="flat">
            <h1>Edit Event #{{ event.id }}</h1>
            <v-spacer></v-spacer>
            <v-btn color="blue" class="text-capitalize mr-2 rounded-lg" variant="elevated" flat @click="submit" :disabled="!form.isDirty || form.processing" :loading="form.processing">Update</v-btn>
...
@/Composables/useEventCreateForm
